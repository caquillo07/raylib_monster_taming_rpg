...
void character_animate(Character *c, const f32 deltaTime) {
	...
	i32 row = 0;
	switch (c->direction) {
		case CharacterDirectionDown:row = 0;
			break;
		case CharacterDirectionLeft:row = 1;
			break;
		case CharacterDirectionRight:row = 2;
			break;
		case CharacterDirectionUp:row = 3;
			break;
	...
	animated_tiled_sprite_update(&c->animatedSprite, deltaTime);
